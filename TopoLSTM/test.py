...
G = preprocessing.load_graph(args.data_dir, node_to_index)
index_to_node = {index:node for node, index in node_to_index.items()}

# load pretrained model
filename = os.path.join(args.data_dir, 'topolstm.pkl')
model = torch.load(filename)


print('Start peredicting')
filename = os.path.join(args.data_dir, 'test.csv')
with open(filename) as f:
    for line in f:
        line = line.strip().split(',')
        node_to_index1 = [node_to_index[node] for node in line]
        with torch.no_grad():
            for i in range(97):
                examples = preprocessing.convert_cascade_to_examples(node_to_index1,G=G,inference=True)
                ...
                prob = select_certain_node_softmax(final_layer,node_to_index1)
