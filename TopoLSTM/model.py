...
class TopoLSTM(nn.Module):
    def __init__(self,data_dir,node_index,hidden_size):
        ...
        self.node_embedding = nn.Parameter(torch.tensor(self.get_embedding_GRU(data_dir), dtype=torch.float))
    ...
    def forward(self, sequence_matrix, sequence_masks_matrix, topo_masks):
        ...
        '''
        embedding_sequence = self.embedding(sequence_matrix)  # x [len(node_index), hidden_size] = [3971, 128]
        ...
        h_sum = (sequence_masks_matrix[:, :, None] * self.h_init).sum(dim=0)  # 256, 128
        lengths = sequence_masks_matrix.sum(dim=0)  # 256
        h_mean = h_sum / lengths[:, None]  # 256, 128
